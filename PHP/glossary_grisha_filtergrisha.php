...
?>
    ...
    <?php
        ...
        /* Datenbank-Abfrage */
        $res = mysqli_query($conn, "SELECT personen.vorname, personen.nachname, personen.weitere_namen, 
        ...
        WHERE personen.grisha_typ = '$grishaSuche'");

        $filt =  mysqli_query($conn, "SELECT * FROM grisha WHERE ID = '$grishaSuche'");
        $f = $filt->fetch_assoc();

        if (mysqli_num_rows($res) >= 1) {  
            /* gewählten Filter ausgeben */
            echo "<div class='glossary-res'><h3>Grisha des Typs " . $f['name'] . "</h3></div>";
            ...
            foreach ($alph as $i){
                ...
                JOIN grisha on personen.grisha_typ = grisha.ID
                JOIN nationen on personen.nation = nationen.ID
                WHERE personen.grisha_typ = '$grishaSuche' AND personen.vorname LIKE '$i%' 
            ...
            echo "<div class='glossary-res'><h3>Keine Grisha des Typs " . $f['name'] . " gefunden</h3></div><br>";
        }
    ?>
    ...
    <div class='glossary-res'>
        <form method='post' action='glossary_grisha.php'>
